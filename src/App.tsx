...
import type { Job, NewJobForm, ConfirmAction, DisputeState, Tab, PostSubTab, SortBy, LeaderboardEntry } from './types'
...
function AppContent() {
  ...
  const { myJobs, setMyJobs, refreshMyJobs } = useMyJobs(address, true)
  ...
  const [newJob, setNewJob] = useState<NewJobForm>({
    title: '', type: 'ML', reward: 50, deadline: '4', description: '', requirements: '', maxWorkers: 3, token: 'zkLTC'
  })
  ...
  useEffect(() => {
    if (tab === 'stats' && entered) fetchLeaderboard(onChainJobs)
  }, [tab, entered, fetchLeaderboard, onChainJobs])

  useEffect(() => {
    if (address && (bio || skills.length > 0)) {
      saveProfile(address, bio, skills)
    }
  }, [address, bio, skills])
  ...
  const postJob = async () => {
    ...
    try {
      ...
      if (newJob.token === 'USDC') {
        const rewardBase = BigInt(rewardPerWorker) * BigInt(10 ** USDC_DECIMALS)
        ...
        showToast(`Approving ${totalBase / BigInt(10 ** USDC_DECIMALS)} USDC... Tx: ${approveHash.slice(0, 10)}...`, 'info')
        ...
        const totalWei = BigInt(rewardPerWorker) * BigInt(maxWorkers) * BigInt(1e18)
      ...
      setNewJob({ title: '', type: 'ML', reward: 50, deadline: '4', description: '', requirements: '', maxWorkers: 3, token: 'zkLTC' })
  ...
  const filteredJobs = [...jobs]
    ...
    .sort((a, b) => sortBy === 'reward' ? b.reward - a.reward : parseInt(a.deadline) - parseInt(b.deadline))
    ...
    <div className="app-container">
      ...
      <div style={{ padding: '40px 24px' }}>
        ...
        {tab === 'stats' && (
          <Stats
            ...
            leaderboard={leaderboard} leaderboardLoading={leaderboardLoading}
      ...
      {showEditProfile && (
        <EditProfileModal
          ...
          onSave={() => { setBio(editBio); setShowEditProfile(false); if (address) saveProfile(address, editBio, skills) }}
        />
      )}
