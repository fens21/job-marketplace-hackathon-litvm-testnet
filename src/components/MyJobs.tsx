...
export function MyJobs({ myJobs, address, onOpenProof, onUnclaim, onRelease, loading, onDispute, onResolveDispute }: {
  ...
  if (myJobs.length === 0) {
    return (
      <div>
        <h2 style={{ fontSize: 28, marginBottom: 24 }}>My Jobs</h2>
    ...
    <div>
      ...
      {activeJobs.length > 0 && (
        <div style={{ marginBottom: 40 }}>
          <div style={{ fontSize: 14, opacity: 0.6, marginBottom: 12 }}>In Progress</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 20 }}>
            {activeJobs.map(job => (
              <div key={job.id} style={{ background: '#111', border: '1px solid #333', borderRadius: 12, padding: 24 }}>
                <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 8 }}>{job.title}</div>
                <div style={{ margin: '16px 0', fontSize: 24, color: '#ffd700', fontWeight: 700 }}>{job.reward} {job.tokenSymbol || 'zkLTC'}</div>
      ...
      {completedJobs.length > 0 && (
        <div style={{ marginBottom: 40 }}>
          <div style={{ fontSize: 14, opacity: 0.6, marginBottom: 12 }}>Proof Submitted &mdash; Awaiting Payment</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 20 }}>
                ...
                <div key={job.id} style={{ background: '#111', border: '1px solid #333', borderRadius: 12, padding: 24, opacity: 0.85 }}>
                  ...
                  {isPoster ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      <div style={{ background: '#2a2a1a', color: '#ffd700', padding: '10px 14px', borderRadius: 6, textAlign: 'center', fontWeight: 600, fontSize: 13 }}>
                        WAITING FOR POSTER TO RELEASE
                      </div>
                      <button onClick={() => onDispute(job)} style={{ background: 'transparent', color: '#f97316', border: '1px solid #f97316', padding: '8px 12px', borderRadius: 6, fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>
                        FILE DISPUTE (Non-Payment)
                      </button>
                    </div>
                  )}
                  <div style={{ background: '#1a3c1a', color: '#4ade80', padding: '10px 14px', borderRadius: 6, textAlign: 'center', fontWeight: 600, fontSize: 13 }}>
      ...
      {myJobs.filter(j => j.status === 'disputed').length > 0 && (
        <div style={{ marginBottom: 40 }}>
          <div style={{ fontSize: 14, opacity: 0.6, marginBottom: 12, color: '#f97316' }}>Disputed</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 20 }}>
            {myJobs.filter(j => j.status === 'disputed').map(job => (
              <div key={job.id} style={{ background: '#111', border: '1px solid #f97316', borderRadius: 12, padding: 24 }}>
                <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 8 }}>{job.title}</div>
                <div style={{ margin: '16px 0', fontSize: 24, color: '#ffd700', fontWeight: 700 }}>{job.reward} {job.tokenSymbol || 'zkLTC'}</div>
                <div style={{ background: '#2a1a0a', color: '#f97316', padding: '10px 14px', borderRadius: 6, textAlign: 'center', fontWeight: 600, fontSize: 13, marginBottom: 8 }}>
                  DISPUTE ACTIVE
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
      ...
      {paidJobs.length > 0 && (
        <div>
          <div style={{ fontSize: 14, opacity: 0.6, marginBottom: 12 }}>Paid</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 20 }}>
            {paidJobs.map(job => (
              <div key={job.id} style={{ background: '#111', border: '1px solid #333', borderRadius: 12, padding: 24, opacity: 0.85 }}>
                <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 8 }}>{job.title}</div>
