  ...
  return (
    <>
      <div style={{ background: 'linear-gradient(90deg, #1a1a2e, #16213e)', padding: 20, borderRadius: 12, marginBottom: 24, border: '1px solid #333' }}>
        <div style={{ color: '#ffd700', fontSize: 14, fontWeight: 600, marginBottom: 4 }}>NEW JOB TYPES AVAILABLE</div>
        <div style={{ fontSize: 18 }}>AI Inference &bull; AI Training &bull; Scientific Simulation &bull; FHE &bull; RAG Pipeline</div>
      </div>

      <div style={{ textAlign: 'center', marginBottom: 20 }}>
        <h1 style={{ fontSize: 32, margin: 0, color: '#fff' }}>Verifiable Compute Marketplace</h1>
        <p style={{ opacity: 0.7, marginTop: 4 }}>Earn zkLTC by running verified compute jobs</p>
      </div>
...
function JobCard({ job, onClaim, onDetail, loading }: { job: Job; onClaim: (job: Job) => void; onDetail: (job: Job) => void; loading: boolean }) {
  ...
  return (
    <div style={{ background: '#111', border: '1px solid #333', borderRadius: 12, padding: 24 }}>
      ...
      <div style={{ opacity: 0.7, fontSize: 14, marginBottom: 16 }}>{job.description}</div>
      <div style={{ margin: '16px 0', fontSize: 26, color: '#ffd700', fontWeight: 700 }}>{job.reward} {job.tokenSymbol || 'zkLTC'}</div>
      <div style={{ fontSize: 13, opacity: 0.7, marginBottom: 16 }}>Posted by {shorten(job.poster)} &bull; {job.claimedCount}/{job.maxWorkers} claimed</div>
